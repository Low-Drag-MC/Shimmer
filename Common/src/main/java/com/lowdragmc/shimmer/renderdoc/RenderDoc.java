package com.lowdragmc.shimmer.renderdoc;
...
/**
 * @see <a href="https://github.com/wisp-forest/owo-lib">owo<a/>
 */
...
public class RenderDoc {
	...
	static {
		...
		if (osName.contains("win") || !osName.contains("mac")) {
			try {
				...
				if (initResult != 1) {
					ShimmerConstants.LOGGER.error("Could not connect to RenderDoc API, return code: {}", initResult);
				} else {
					...
					ShimmerConstants.LOGGER.info("Connected to RenderDoc API v" + major.getValue() + "." + minor.getValue() + "." + patch.getValue());
				}
			} catch (UnsatisfiedLinkError ignored) {
			}
		}
		ShimmerConstants.LOGGER.info("renderDoc available:" + (apiInstance == null ? "failed" : "success"));
	...
	public enum Key {
		...
		public static @NotNull Key fromGLFW(int glfw) {
